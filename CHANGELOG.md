...
## v3.4.1 — 修复双形态 double-load 回归：刷新后渲染器全灭（运行时标记 version: 8）

- **严重回归修复**：v3.3.0 引入的版本守卫"先 disable 旧实例再检查版本"，在**插件形态 + 磁盘补丁并存**时被同版本 double-load 触发——刷新后插件内核先启动渲染，磁盘补丁内核随后加载即把前者杀掉又因版本相同直接 return，**没有任何替补实例，全部围栏退回代码块**（用户实测：刷新前正常、刷新后全灭）。
- 修复：同版本/更新版本已在运行时**直接让位**（不 disable）；仅升级场景（旧 < 新）执行替换。双形态并存从此在任何加载顺序下都恰好存活一个实例。
- 内核升版 version 8（缓存击穿 `?v=8` 强制浏览器拉新）；bundle 同步重建。
