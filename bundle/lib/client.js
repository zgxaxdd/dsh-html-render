...
(function () {
  ...
  /* P0-1 资产基座可配置：磁盘补丁形态默认 /dsh-html/katex/；插件形态由
   * client 包装器预先设置 window.__dshHtmlAssetsBase（指向插件自有路由）。 */
  var ASSETS_BASE = window.__dshHtmlAssetsBase || '/dsh-html/katex/'
  ...
  function ensureKatex(cb, onFail) {
    ...
    sc.src = ASSETS_BASE + 'katex.min.js'
    sc.async = true
    sc.onload = function () {
      katexState = 'ok'
      katexAttempts = 0
  ...
  function ensureKatexCss(cb) {
    ...
    katexCssPromise = fetch(ASSETS_BASE + 'katex.min.css')
      .then(function (r) { return r.ok ? r.text() : null })
      .then(function (txt) {
        if (!txt) { katexCssPromise = null; return null }
        var out = txt.replace(
          /url\(["']?fonts\/([^)"']+)["']?\)/g,
          'url(' + ASSETS_BASE + 'fonts/$1)'
        )
  ...
  function wrapFragment(parent, text) {
    ...
    var id = ++mountSeq
    wrap._dshFrameId = id
    liveFramesAdd(id, { iframe: frame, onHeight: function () {}, id: id, fragText: text })
    try {
  ...
  /* ------------------------------------------------------------------ *
   * P1-3 主题即时跟随：宿主深浅切换（系统偏好或根元素 class/属性变化）
   * → 立即失效调色板缓存 → 全量重渲染已挂载围栏（0 延迟，替代 10s TTL）。
   * ------------------------------------------------------------------ */
  var themeObserver = null
  function invalidateTheme() {
    paletteCache.t = 0
    for (var m of mounts.values()) {
      if (m.oversized) continue
      try { renderFrame(m) } catch (e) { dbg(e) }
    }
    /* 通道二片段：按缓存的原文重建（主题色烙在 body style 里）。 */
    for (var rec of liveFrames.values()) {
      if (rec.fragText != null) {
        try { rec.iframe.srcdoc = wrapDocument(rec.fragText, null, rec.id, false) } catch (e) { dbg(e) }
      }
    }
  }
  ...
  window.__dshHtmlRenderer.disable = function () {
    ...
    if (themeObserver) { try { themeObserver.disconnect() } catch (e) { dbg(e) } }
